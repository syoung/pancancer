...
def gen_unaligned_bam(bam_filename, analysis_id, metadata, specimen_dict, work_dir, output_dir, num_processes=4, logger=default_logger ):
    ...
    try:
        cmd = "bamtofastq outputperreadgroup=1 gz=1 level=1 inputbuffersize=2097152000 outputdir=%s `mktemp -p %s bamtofastq_XXXXXXXXX` < %s" %(output_dir, work_dir, bam_filename)
